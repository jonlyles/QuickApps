...
class ContentsController extends NodeAppController {
        ...
        if ( isset($this->data['Node']['update']) ){
            if ( isset($this->data['Items']['id']) ){
                ...
                switch ($this->data['Node']['update']) {
                    case 'publish': default: $data = array('field' => 'status', 'value' => 1); break;
                    case 'unpublish': $data = array('field' => 'status', 'value' => 0); break;
                    case 'promote': $data = array('field' => 'promote', 'value' => 1); break;
                    case 'demote': $data = array('field' => 'promote', 'value' => 0); break;
                    case 'sticky': $data = array('field' => 'sticky', 'value' => 1); break;
                    case 'unsticky': $data = array('field' => 'sticky', 'value' => 0); break;
                }
	...
	function admin_add($node_type_id){
        if ( !empty($this->data) ){
        ...
        // Fix
        foreach ( $type['Field'] as &$f ) $f['settings'] = is_string($f['settings']) ? unserialize($f['settings']) : $f['settings'];
    ...
    function admin_clear_cache($slug){
        return Cache::delete("node_{$slug}");
    }
    
    function __setLangVar(){
        $langs = array();
