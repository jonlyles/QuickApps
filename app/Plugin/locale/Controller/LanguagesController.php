...
class LanguagesController extends LocaleAppController {
	var $name = 'Languages';
	var $uses = array('Locale.Language');
        ...
        if ( isset($this->data['Language']['update']) ){
            if ( isset($this->data['Items']['id']) ){
                ...
                switch ($this->data['Language']['update']) {
                    case 'enable': default: $data = array('field' => 'status', 'value' => 1); break;
                    case 'disable': $data = array('field' => 'status', 'value' => 0); break;
                }
                    
                foreach( $this->data['Items']['id'] as $key => $id ){
                    if( in_array($id, array(1, $this->__languageIdByCode(Configure::read('Variable.default_language')) )) ) continue;
    ...
    function __languageCodeById($id){
    ...
    function __languageIdByCode($code){
        $l = Configure::read('Variable.languages');
